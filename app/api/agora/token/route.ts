...
export async function POST(req: NextRequest) {
  ...
  if (!APP_ID) {
    console.error("❌ NEXT_PUBLIC_AGORA_APP_ID not set in environment variables");
    return NextResponse.json(
      { error: "Agora APP_ID not configured. Add NEXT_PUBLIC_AGORA_APP_ID to environment variables." },
      { status: 500 }
    );
  }
  ...
  if (!APP_CERTIFICATE) {
    ...
    console.error(`⚠️  This is CRITICAL for token generation - calls will fail without it`);
    console.error(`✅ APP_ID is set: ${APP_ID.substring(0, 5)}...`);
    return NextResponse.json(
      { error: "Agora APP_CERTIFICATE not configured. Add AGORA_APP_CERTIFICATE to environment variables." },
      { status: 500 }
    );
  }
  
  if (!channelName || !uid) {
    console.warn("⚠️  Token request missing required params", { channelName, uid });
    return NextResponse.json(
