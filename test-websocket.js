...
const log = {
  ...
  user: (userId, msg) => console.log(`${colors.yellow}👤 [${userId}] ${msg}${colors.reset}`),
};
...
async function runTests() {
  ...
  try {
    ...
    assert(user1Received, `User 2 received message (${clients[TEST_USERS[1]].receivedMessages.length} messages)`);
    assert(user2Received, `User 3 received message (${clients[TEST_USERS[2]].receivedMessages.length} messages)`);
    ...
    const typingUpdates1 = clients[TEST_USERS[1]].getReceivedCount('typing-update');
    const typingUpdates2 = clients[TEST_USERS[2]].getReceivedCount('typing-update');
    ...
    assert(totalMessages >= 3, `User 3 received at least 3 messages (got ${totalMessages})`);
